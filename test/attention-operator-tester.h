...
class AttentionOperatorTester {
 public:
  ...
  void TestF16() const {
    ...
    std::uniform_real_distribution<float> f32dist(0.1, 0.9f);
    std::uniform_real_distribution<float> scaledist(0.2f, 2.0f);

    std::vector<uint16_t> query(XNN_EXTRA_BYTES / sizeof(uint16_t) + batch_size() * query_tokens() * channels());
    std::vector<uint16_t> key(XNN_EXTRA_BYTES / sizeof(uint16_t) + batch_size() * key_value_tokens() * channels());
    std::vector<uint16_t> value(XNN_EXTRA_BYTES / sizeof(uint16_t) + batch_size() * key_value_tokens() * channels());
    std::vector<uint16_t> scale(XNN_EXTRA_BYTES / sizeof(uint16_t) + channels());
    std::vector<uint16_t> mask(XNN_EXTRA_BYTES / sizeof(uint16_t) + query_tokens() * key_value_tokens());
    std::vector<uint16_t> output(batch_size() * query_tokens() * channels());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      for (size_t b = 0; b < batch_size(); b++) {
        ...
        for (size_t n = 0; n < query_tokens(); n++) {
          for (size_t k = 0; k < channels(); k++) {
            q_scaled[n * channels() + k] =
              fp16_ieee_to_fp32_value(query[b * query_tokens() * channels() + n * channels() + k]) *
              fp16_ieee_to_fp32_value(scale[k]);
          }
        }

        std::vector<uint16_t> logits(query_tokens() * key_value_tokens(), 0);
          ...
          for (size_t n_1 = 0; n_1 < key_value_tokens(); n_1++) {
            for (size_t ki = 0; ki < channels(); ki++) {
              logits[n_0 * key_value_tokens() + n_1] =
                fp16_ieee_from_fp32_value(
                  fp16_ieee_to_fp32_value(logits[n_0 * key_value_tokens() + n_1]) +
                    (q_scaled[n_0 * channels() + ki]) *
                    fp16_ieee_to_fp32_value(key[b * key_value_tokens() * channels() + n_1 * channels() + ki]));
            }
            if (cap_type() == xnn_attention_logits_cap_type_tanh) {
              ...
              logits[n_0 * key_value_tokens() + n_1] =
                  fp16_ieee_from_fp32_value(
                    std::tanh(
                        fp16_ieee_to_fp32_value(logits[n_0 * key_value_tokens() + n_1]) / cap_value()) * cap_value());
            }
            // Mask.
            logits[n_0 * key_value_tokens() + n_1] =
                fp16_ieee_from_fp32_value(
        ...
        std::vector<uint16_t> weights(query_tokens() * key_value_tokens(), UINT16_C(0));
        ...
        for (size_t i = 0; i < query_tokens(); i++) {
          ...
          for (size_t j = 0; j < key_value_tokens(); j++) {
            float prev_m = mv;
            mv = std::max(prev_m, fp16_ieee_to_fp32_value(logits[i * key_value_tokens() + j]));
            dv = dv * exp(prev_m - mv) + exp(fp16_ieee_to_fp32_value(logits[i * key_value_tokens() + j]) - mv);
          }
          for (size_t j = 0; j < key_value_tokens(); j++) {
            weights[i * key_value_tokens() + j] =
                fp16_ieee_from_fp32_value(exp(fp16_ieee_to_fp32_value(logits[i * key_value_tokens() + j]) - mv)/ dv);
          }
        }
          ...
          for (size_t nj = 0; nj < key_value_tokens(); nj++) {
            for (size_t di = 0; di < channels(); di++) {
              output_ref[b * query_tokens() * channels() + ni * channels() + di] +=
                  fp16_ieee_to_fp32_value(weights[ni * key_value_tokens() + nj]) *
                  fp16_ieee_to_fp32_value(value[b * key_value_tokens() * channels() + nj * channels() + di]);
            }
          }
        }
      }
      ...
      const xnn_status status = xnn_create_scaled_dot_attention_ntc_f16(
          cap_type(),
          &cap_tanh_params,
          /*flags=*/0,
          &attention_op);
      ...
      ASSERT_EQ(xnn_status_success,
                xnn_reshape_scaled_dot_attention_ntc_f16(
                  attention_op,
      ...
      ASSERT_EQ(xnn_status_success,
                xnn_setup_scaled_dot_attention_ntc_f16(
                  attention_op,
        ...
        for (size_t i = 0; i < query_tokens(); i++) {
          for (size_t j = 0; j < channels(); j++) {
            EXPECT_NEAR(output_ref[b * query_tokens() * channels() + i * channels() + j],
                        fp16_ieee_to_fp32_value(output[b * query_tokens() * channels() + i * channels() + j]),
                        1e-1)
                << " batch : " << b << " / "  << batch_size()
